...
# Load your AdaBoost model file
try:
    model = joblib.load('adaboost1_pkl.pkl')
except FileNotFoundError:
    print("Error: 'adaboost1_pkl.pkl' file not found in the root directory!")

# All-in-one HTML structure and CSS UI styling
COMBINED_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .results-panel { padding: 2.5rem; background-color: #fafbfd; display: flex; align-items: center; justify-content: center; width: 100%; }
        ...
        .output-card { width: 100%; text-align: center; padding: 3rem 2rem; border-radius: 16px; background: var(--card-bg); border: 1px solid var(--border-color); box-shadow: var(--shadow-sm); }
        .output-icon { width: 70px; height: 70px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.5rem auto; font-size: 1.8rem; }
        ...
        .prediction-value { font-size: 3rem; font-weight: 800; color: var(--accent-success); margin: 1rem 0; letter-spacing: -1px; animation: scalePop 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
    ...
    <div class="dashboard-container">
        ...
        <main class="workspace">
            <form method="POST" action="/" class="prediction-form">
                <div class="form-section">
                    ...
                    <div class="grid-layout">
                        <div class="input-group">
                            <label for="Hours_Coding">Coding Duration (Hours)</label>
                            <div class="input-wrapper">
                                <i class="fa-regular fa-clock field-icon"></i>
                                <input type="number" step="0.1" name="Hours_Coding" id="Hours_Coding" min="0" max="24" required placeholder="e.g. 6.5" value="{{ form.get('Hours_Coding', '') }}">
                            </div>
                        </div>
                        <div class="input-group">
                            <label for="AI_Usage_Hours">AI Copilot Usage (Hours)</label>
                            <div class="input-wrapper">
                                <i class="fa-solid fa-microchip field-icon"></i>
                                <input type="number" step="0.1" name="AI_Usage_Hours" id="AI_Usage_Hours" min="0" max="24" required placeholder="e.g. 3.0" value="{{ form.get('AI_Usage_Hours', '') }}">
                            </div>
                        </div>
                        <div class="input-group">
                            <label for="Lines_of_Code">Lines of Code Written</label>
                            <div class="input-wrapper">
                                <i class="fa-solid fa-terminal field-icon"></i>
                                <input type="number" name="Lines_of_Code" id="Lines_of_Code" min="0" required placeholder="e.g. 250" value="{{ form.get('Lines_of_Code', '') }}">
                            </div>
                        </div>
                        <div class="input-group">
                            <label for="Commits">Git Commits Pushed</label>
                            <div class="input-wrapper">
                                <i class="fa-solid fa-code-commit field-icon"></i>
                                <input type="number" name="Commits" id="Commits" min="0" required placeholder="e.g. 4" value="{{ form.get('Commits', '') }}">
                            </div>
                        </div>
                        <div class="input-group wide-column">
                            <label for="Bugs_Reported">Bugs Linked / Logged</label>
                            <div class="input-wrapper">
                                <i class="fa-solid fa-bug field-icon"></i>
                                <input type="number" name="Bugs_Reported" id="Bugs_Reported" min="0" required placeholder="e.g. 2" value="{{ form.get('Bugs_Reported', '') }}">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section">
                    <h3 class="section-title"><i class="fa-solid fa-bolt"></i> Focus & Cognitive Environment</h3>
                    <div class="grid-layout">
                        <div class="input-group">
                            <label for="Sleep_Hours">Sleep Quality (Hours)</label>
                            <div class="input-wrapper">
                                <i class="fa-solid fa-bed field-icon"></i>
                                <input type="number" step="0.1" name="Sleep_Hours" id="Sleep_Hours" min="0" max="24" required placeholder="e.g. 7.5" value="{{ form.get('Sleep_Hours', '') }}">
                            </div>
                        </div>
                        <div class="input-group">
                            <label for="Distractions">Distraction Level (Scale 0-10)</label>
                            <div class="input-wrapper">
                                <i class="fa-solid fa-bell-slash field-icon"></i>
                                <input type="number" name="Distractions" id="Distractions" min="0" max="10" required placeholder="0 = None, 10 = High" value="{{ form.get('Distractions', '') }}">
                            </div>
                        </div>
                        <div class="input-group">
                        ...
                        <div class="input-group">
                            <label for="Stress_Level">Perceived Stress Level</label>
                            <div class="input-wrapper">
            ...
            <div class="results-panel">
                ...
                {% else %}
                    <div class="output-card empty">
                        ...
                        <p>Fill out the telemetry logs on the left panel to execute your engineering productivity calculation pipeline.</p>
                    </div>
                {% endif %}
            </div>
        </main>
...
def index():
    ...
    if request.method == 'POST':
        try:
            ...
            # Map user-friendly dropdown options to numbers matching model training values
            categorical_mapping = {'Low': 0.0, 'Medium': 1.0, 'High': 2.0}
            
            # Organize exactly the 9 inputs matching adaboost1_pkl.pkl feature expectations
            features = [
            ...
            # Inference execution via loaded AdaBoost classifier
    ...
    return render_template_string(COMBINED_TEMPLATE, prediction=prediction, error_message=error_message, form=form_data)
